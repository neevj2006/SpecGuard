...
Selecting model mode transmits the requirement and selected repository snippets to the provider. Provider credentials are not persisted by SpecGuard. Model responses must reference supplied evidence IDs; malformed responses, unknown citations, and provider failures downgrade to abstention. Model confidence is uncalibrated and separate from test status. Provider usage and cost are currently unknown (`null`) rather than estimated as zero.

Repository content is delimited as untrusted data in the verifier contract. This reduces instruction confusion but is not a claim that prompt injection is solved. Review source before sending it to a provider; automatic secret detection and redaction are not yet implemented.
